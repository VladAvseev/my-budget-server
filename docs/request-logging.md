...
- **Логирование запросов:** `requestLoggingMiddleware` пишет каждый HTTP-запрос
  ...
  основной объём таблицы и светили данные; текст ошибки для статусов 4xx извлекается
  на лету из envelope `{ error: { message } }` (обрезается до 512 символов).
  Для статусов 5xx клиенту уходит общий текст «Внутренняя ошибка сервера», а в колонку
  `error` пишется настоящее сообщение (его кладёт `errorMiddleware` в `res.locals.serverError`;
  stderr при этом содержит сообщение, pg-детали и стек). UUID- и числовые сегменты пути пишутся как `:id`
