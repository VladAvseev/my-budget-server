-- Шаг 8: проверка целостности после шагов 1–7; при успехе снимок удаляется.
-- Выполнять в транзакции: при падении проверки всё откатывается.
begin;
...
-- Проверка инварианта денег (переводы в sum() не участвуют).
do $$
declare
  v_bad text;
begin
